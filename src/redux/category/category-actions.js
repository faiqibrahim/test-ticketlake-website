...
export const getAllCategories = (cb, version = "v1") => {
  return (dispatch) => {
    ...
    if (allCategoriesResponse && allCategoriesResponse.length) {
      setAllCats(allCategoriesResponse);
    } else {
      axios
        .get("/categories/get-all-categories", version)
        .then((response) => {
          allCategoriesResponse = response.data.data || [];
          setAllCats(allCategoriesResponse);
        })
        .catch((err) => {
...
export const fetchParentCategories = (version = "v2") => {
  return (dispatch) => {
    ...
    axios
      .get("/categories/get-all-parent-categories", version)
      .then((response) => {
...
export const fetchChildCategories = (parentId, cb, version = "v2") => {
  return (dispatch) => {
    dispatch(setCategoryProcessing(true));
    axios
      .get(`/categories/get-all-sub-categories/${parentId}`, version)
      .then((response) => {
        cb && cb();
        dispatch(setCategoryProcessing(false));
