...
class HeaderNavMenu extends Component {
    ...
    lpFunctions = (item, i, link, breadCrumbState) => {
        ...
        if (item.children.length > 0) {
            ...
            return (
                <DropdownItem key={i} onClick={() => this.closeMenu(item._id, navState)}>
                    {item.name}
                </DropdownItem>
            )
        }
    ...
    renderDisplayMenu = () => {
        ...
        return categoryState && categoryState.map((item, i) => {
            if (i < navLimit) {
                let { selectedCategories } = this.props;
                let breadCrumbsState = [];

                selectedCategories = selectedCategories.filter(category => category.title === item.name.trim());
                ...
                return (
                    item.children.length > 0 ?
                        ...
                        :
                        <DropdownItem key={i} className="border-bottom hee"
    ...
    renderMobileMoreCategories = (keyIndex) => {
        ...
        if (categoryState && categoryState.length >= navLimit) {
            for (let i = navLimit; i < categoryState.length; i++) {
                ...
                navJsx.push(
                    <Fragment key={i}>
                        <DropdownItem className="border-bottom hee"
                            onClick={() => this.closeMenu(item._id, navState)}>
                            {item.name}
                        </DropdownItem>
                    </Fragment>
            ...
            return (
                <UncontrolledDropdown key={keyIndex} nav inNavbar style={{ background: '#e8e8e8' }}
                    className="border-bottom mh">
    ...
    onWebsiteMenuChildClick = (item) => {
        this.props.history.push(`/movies/sub-category/${item._id}`)
        sessionStorage.setItem('subCategoryItem', JSON.stringify(item));
                ...
                <Fragment key={i}>
                    <li>
                        ...
                        <NavLink to={{
                            pathname: pathURL,
                            state: { parentCategory: item, breadCrumbs: breadCrumb, navLink: true }
                        }} onClick={() => this.onWebsiteMenuChildClick(item)}>{item.name}<span><i
                            className="fas fa-caret-down" /></span></NavLink>
                    ...
                    <li key={i}>
                        ...
                        <NavLink
                            to={{
                                pathname: pathURL,
                                state: { parentCategory: item, breadCrumbs: breadCrumb, navLink: true }
                            }} onClick={() => this.onWebsiteMenuChildClick(item)}>{item.name}</NavLink>
                    </li>
                </Fragment>
            )
        }
                    ...
                    item.children.length > 0 ?
                        <li key={i}>
                            ...
                            <NavLink to={{
                                pathname: pathName,
                                state: { parentCategory: item, breadCrumbs: breadCrumbsState, navLink: true }
                            }} onClick={() => this.onNavClick(item)}>{item.name}<span><i
                                className="fas fa-caret-down" /></span></NavLink>
    ...
    renderWebsiteMoreCategories = (keyIndex) => {
        ...
        if (categoryState && categoryState.length > navLimit) {
            ...
            return (
                <li key={keyIndex}>
                    <a href={hrefVal}>More<span><i
                        className="fas fa-caret-down" /></span></a>
                    <ul className="sub-menu">
                        {navJsx}
                    </ul>
                </li>
            ...
            <>
                <div className='navbarHolder'>
                    <Navbar expand="md" className="website-mobile-menu">
                        <div className={this.state.isOpen ? 'nav-button-wrap opened' : 'nav-button-wrap color-bg'}>
                            ...
                            <Nav navbar className={"header-mob-nav"}>
                                {!this.state.loaded && !sessionState ?
                                    ...
                                    <>
                                        ...
                                        {this.renderMobileMoreCategories(menu.length + selectedCategories.length)}
                                        <NavItem key={menu.length + selectedCategories.length + 1}
                                            className="hee border-bottom">
                                            <NavLink to={'/about-us'} onClick={this.toggle}>About Us</NavLink>
                                        </NavItem>
                                        <NavItem key={menu.length + selectedCategories.length + 2} className="hee">
                                            <NavLink to='/contact-us' onClick={this.toggle}>Contact</NavLink>
                                        </NavItem>
                                    </>
                                }
                            </Nav>
