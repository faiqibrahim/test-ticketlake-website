...
import {MTN_IMG, VODA_IMG, AIRTEL_IMG, GLO_IMG, NETWORK_IMG} from '../../utils/config';
...
class CheckoutStepTwo extends Component {
    ...
    onContinueClick = (props) => {
        if (!this.state.selectedNetwork || !this.state.phone){
            NotificationManager.error("Please Provide Phone and Network Information!", '', 3000);
        }else {
            const {phone, network} = this.state;
            this.props.sendSmsOTP(phone.replace(/\s/g, ''), network, props.setCheckoutStepThree);
        }
    };
                        ...
                        <div className={'static-box'}>
                            <div className={'row'}>
                                ...
                                <div className={'col-lg-12'} style={{marginTop: '4%'}}>
                                    <ReactPhoneInput
                                        inputStyle={{width: '355px', height: '51px'}}
                                ...
                                <div className={'col-lg-12'}>
                                    <button className={'simpleRedBtn'} onClick={() => this.onContinueClick(props)}>
                                        Continue
                                    </button>
                                </div>
                            </div>
