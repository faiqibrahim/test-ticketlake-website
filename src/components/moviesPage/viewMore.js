...
class ViewMore extends Component {
    ...
    setEventsData = () => {
        ...
        switch (sectionId) {
            case 1:
                this.props.getTrendingEvents(categoryId, true, this.state.currentPage, "", this.state.pageSize, (data) => {
                    let eventData = data && data.data;
                    this.setState({events: eventData, pageTitle: "Trending Events", processing: false, totalPages: data && data.totalPages});
                    sessionStorage.setItem('sectionId', null);
                });
                break;
            case 2:
                this.props.getShowingInCinemasEvents(categoryId, true, this.state.currentPage, "", this.state.pageSize, (data) => {
                    let eventData = data && data.data;
                    this.setState({events: eventData, pageTitle: "Showing in Cinema", processing: false, totalPages: data && data.totalPages});
                    sessionStorage.setItem('sectionId', null);
                });
                break;
            case 3:
                this.props.getUpcomingEvents(categoryId, true, this.state.currentPage, "", this.state.pageSize, (data) => {
                    let eventData = data && data.data;
                    this.setState({events: eventData, pageTitle: "Upcoming Events", processing: false, totalPages: data && data.totalPages});
                    sessionStorage.setItem('sectionId', null);
                });
                break;
            case 4:
                this.props.getNearByEvents(categoryId, true, this.state.currentPage, "", this.state.pageSize, (data) => {
                    let eventData = data && data.data;
                    this.setState({events: eventData, pageTitle: "Shows Near You", processing: false, totalPages: data && data.totalPages});
                    sessionStorage.setItem('sectionId', null);
                });
                break;
            default : {
                if(subCategoryItem){
                    this.props.getSubCategoriesEvents(subCategoryItem._id, true, this.state.currentPage, "", this.state.pageSize, (data) => {
                        let eventData = data && data.data;
                        this.setState({events: eventData, pageTitle: subCategoryItem.name, processing: false, totalPages: data && data.totalPages})
                    });
                }else {
    ...
    onMouseOut = (e, youtubeVideoId) => {
        e.preventDefault();
        if (youtubeVideoId) {
            this.setState({playTrailerOnMouseHover: false, playVideoWithIndexes: []})
        }
    };
                                                        ...
                                                        <>
                                                            {
                                                                this.state.events.map((event, index) => {
                                                                    ...
                                                                    let imageUrl = event.bannerImageKey && event.bannerImageKey.imageUrl;
                                                                    ...
                                                                    return (
                                                                        <CardWithHoverEffect index={index}
                                                                                             imageUrl={imageUrl}
                                                                                             title={title}
                                                                                             ...
                                                                                             youtubeVideoId={youtubeVideoId}
                                                                                             playTrailerOnMouseHover={this.state.playTrailerOnMouseHover}
                                                                                             playVideoWithIndexes={this.state.playVideoWithIndexes}
...
const connectedComponent = connect(mapStateToProps, {
        getTrendingEvents, getShowingInCinemasEvents, getNearByEvents, getUpcomingEvents, getSubCategoriesEvents
    }
)(ViewMore);
