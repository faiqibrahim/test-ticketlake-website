...
class Wishlist extends Component {
  ...
  filterWishlistEvents = () => {
    ...
    if (status === "upcoming") {
      const upcomingEvents = filteredList.filter(
        (event) =>
          event.eventDateTimeSlot.eventEndTime >= new Date().toISOString()
      );
      this.setState({ wishList: upcomingEvents });
    } else if (status === "expired") {
      const expiredEvents = filteredList.filter(
        (event) =>
          event.eventDateTimeSlot.eventEndTime < new Date().toISOString()
      );
                  ...
                  {wishListEvents && wishListEvents.length > 0 ? (
                          ...
                          return (
                            <>
                              <DefaultCard
                                ...
                                isWishList={true}
                                wishlistLink={() =>
                                  this.wishListToggle(data.eventSlotId)
                                }
                                cardLink={"#"}
                                cardAddress={
                    ...
                    <div className={"Error-msg-wrp w100"}>
                      ...
                      <span className={"Error-sub-heading"}>
                        There are no liked events in your wishlist.
                      </span>
                    </div>
                  )}
                </div>
  ...
  render() {
    ...
    breadCrumbs.push(
      <BreadcrumbsItem to="/user/wishlist">User Wishlist</BreadcrumbsItem>
    );

    const { status, wishList: filteredWishList } = this.state;
    const { ticketPagination = {}, userWallet, wishList } = this.props;
...
const mapStateToProps = (state) => {
  return {
    ...
    wishListPage:
      state.wishlist.wishList !== null ? state.wishlist.wishList.page : 1,
    totalPages:
      state.wishlist.wishList !== null
        ? state.wishlist.wishList.totalPages
        : null,
  };
};
