...
import {setCopounValue, setBillSummary, resetCopounValue, setCouponProcessing} from '../../redux/ticket/ticket-actions';
...
class BillSummary extends Component {
    ...
    validateCoupon = (sum) => {
        ...
        if (this.state.enterCouponCode.trim().length > 1) {
            axios.post('/coupons/validate-coupons', {
    ...
    getWalletDisplay = (step, wallet, sum, totalBill, currency) => {
        ...
        if (step >= 2 && wallet) {
            ...
            if (couponValue) {
                if (couponValue && couponValue.discountType === 'percentage') {
                    sum = sum - (sum * (couponValue.discountValue / 100));
                    sum = sum >= 0 ? sum : 0;

                } else if (couponValue) {
                    sum -= couponValue.discountValue;
                    sum = sum >= 0 ? sum : 0;
                }
            }
            ...
            if (wallet.availableBalance > 0) {
                ...
                if (sum < wallet.availableBalance || sum === wallet.availableBalance) {
                    ...
                    remaining = wallet.availableBalance - sum;
                    walletCost = sum;
                } else {
                    remaining = 0;
                    walletCost = wallet.availableBalance;
                }

            } else if (walletInner > 0) {
                if (sum < walletInner || sum === walletInner) {
                    remaining = walletInner - sum;
                    walletCost = walletInner - sum;

                } else {
                    remaining = 0;
                    walletCost = walletInner;
                }
            }
                ...
                <div className='ticketPriceDetail'>
                    <div className='row'>
                        <div className='col-md-6'>
                            ...
                            <small>GHS{remaining ? remaining.toFixed(2) : 0} Remaining</small>
                        </div>
                        <div className='col-md-6' style={{textAlign: 'right'}}>
                            <span>
                                -{currency}{walletCost ? walletCost.toFixed(2) : 0}
                            </span>
                        </div>
                    </div>
                </div>
    ...
    hasEmptySeat = () => {
        ...
        for (let index = 0; index < assignedSeatsForDisplay.length; index++) {
            ...
            isEmpty = userInfo.name === '' || userInfo.email === '' || validator.isEmail(userInfo.email) === false || userInfo.phoneNumber === '';
    ...
    hasEmptyPass = () => {
        ...
        for (let index = 0; index < passesAssignedSeatsForDisplay.length; index++) {
            ...
            isEmpty = userInfo.name === '' || userInfo.email === '' || userInfo.phoneNumber === '';
    ...
    handleRemoveCouponClick = () => {
        this.setState({enterCouponCode: '', isDisable: true, copounData: {}}, () => {
            this.props.resetCopounValue();

            if (this.props.currentStep === 2) {
                this.props.setBillSummary(this.props.billSummary, this.state.initialWallet);
            } else {
    ...
    getCouponSection = () => {
        ...
        if (typeof couponValue === 'object') {
            ...
            discountValue = discountType === 'fixed' ? `GHS${discountValue}` : `${discountValue}%`;
                ...
                <div className='ticketPriceDetail'>
                    <div className='row'>
                        <div className='col-md-6'>
                            ...
                            <small>Type: <b>{capitalize(discountType)}</b></small>
                            <small>Discount: <b>{`${discountValue}`}</b></small>
                        </div>
                        <div className='col-md-6 coupon-right-div' style={{textAlign: 'right'}}>
                            <div>
                                <span>
                                    -{this.props.currency}{couponDeduction.toFixed(2)}
                                 </span>
                                {currentStep <=2 ?
                                    <i className="fa fa-trash" onClick={this.handleRemoveCouponClick}/>
                                    : null
                                }
                            </div>
                        </div>
    ...
    onFinish = e => {
        ...
        if (!err) {
            ...
            .catch(err => {
                console.log(err.response!==undefined?err.response.data._error:'Error')
                this.setState({
                  optError:err.response!==undefined?err.response.data._error:'Error'
                })
    ...
    getDisplay = () => {
        ...
        if (this.props.currentStep === 1) {
            ...
            if(this.props.user!==null && this.props.prodileData!==undefined && this.props.prodileData===true){
                    displayButton.push(<button className='checkoutButton' key={0} onClick={this.props.forward}>Continue</button>)
            }else{
                displayButton.push(<button className='checkoutButton' onClick={this.showModal}>Continue</button>)
            }
            ...
            let {assignedSeatsForDisplay, passesAssignedSeatsForDisplay} = this.props;

            let isEmptySeat = assignedSeatsForDisplay && assignedSeatsForDisplay.length && this.hasEmptySeat();
            let isEmptyPass = passesAssignedSeatsForDisplay && passesAssignedSeatsForDisplay.length && this.hasEmptyPass();
            ...
            displayButton.push(
                <button disabled={isButtonDisable} key={0} className={`checkoutButton ${disableClass}`}
        ...
        if (this.props.billSummary) {
            ...
            let couponInputValue = isCouponExist ? couponValue.promoCode : enterCouponCode;
                    ...
                    <div className="col-md-12">
                                        ...
                                        displayData.push(
                                            <div className='ticketPriceDetail' key={i}>
                                                <div className='row'>
                                                    ...
                                                    <div className='col-md-6' style={{textAlign: 'right'}}>
                                                        <span>
                                                            {this.props.currency}{(item.ticketClassPrice * item.ticketClassQty).toFixed(2)}
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    displayData.push(
                                        <div className='ticketPriceDetail' key={i}>
                                            <div className='row'>
                                                ...
                                                <div className='col-md-6' style={{textAlign: 'right'}}>
                                            <span>
                                                {this.props.currency}{(item.passPrice * item.ticketClassQty).toFixed(2)}
                                            </span>
                                                </div>
                                            </div>
                                ...
                                <div className='ticketTotalPrice'>
                                    <div className='row'>
                                        <div className='col-md-12 custom-form'>
                                            <input disabled={isCouponExist} type="text" className="coupen-field"
                                            ...
                                            <button style={{cursor: isDisable && 'default'}} type="button"
                                                    className='checkoutButton coupen-apply-btn'
                                                    onClick={this.validateCoupon} disabled={isDisable}>
                                                Apply
                                            </button>
                                        </div>
                                        <div className='col-md-12 error-msg-wrp' style={{textAlign: 'left'}}>
                                            ...
                                            {this.props.couponProcessing?
                                                <small style={{color: 'red'}}>Validating Coupon....</small>
                                                :
                                                null
                        ...
                        <div className='ticketTotalPrice'>
                            <div className='row'>
                                ...
                                <div className='col-md-6'>
                                    <span>{this.props.currency}{this.props.totalBill}
                                    </span>
                                </div>
                            </div>
                        </div>
        ...
        return (
            <>
                ...
                <Modal
                      ...
                      >
                        <p><img alt='img' src={window.location.origin + '/images/nav-logo.svg'}/></p>
                        {!this.state.showOtp?
                            <>
                                <p><h3>Please verify your email </h3>
                                    Please verify your email <span style={{fontWeight:'500', fontSize:'14px'}}>{this.props.user!==null?this.props.user.email:'no email found'} </span>linked with your Ticketlake account
                                    Once email is verified, you will be able to make your Wishlist, set up wallet and access more features. </p>
                                <p>
                                    <button className="btn btn-danger buttonDefault defaultBackground" onClick={this.showOtp}>Verify Now</button>
                                </p>
                                <p>
                                    <NavLink to="/user/profile" style={{color:'red'}}>Use a different email?</NavLink>
                                </p>
                            </>
                            :
                            <>
                            <h3>Verification</h3>
                                <Form
                                    ...
                                    <Form.Item className="custom-form">
                                        <Button type="primary" htmlType="submit" className="log-submit-btn" style={{maxWidth: '109px', height: '47px'}}>
                                            Verify
                                        </Button>
                                        {/* <Button type="primary"className="log-submit-btn" style={{maxWidth: '139px', height: '47px', marginLeft:'20px'}} onClick={this.handleCancel}>
                                            Cancel
                                        </Button> */}
                                    </Form.Item>
                                </Form>
...
const connectedComponent = connect(mapStateToProps, {fetchUserProfile, setCopounValue, setBillSummary, resetCopounValue,setCouponProcessing})(OrgAddForm);
