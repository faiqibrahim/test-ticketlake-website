  ...
  getBreadCrumbs = () => {
    return (
      <>
        ...
        <BreadcrumbsItem
          to={"/event/nearby-cinemas/" + this.props.match.params.id}
        >
          Cinema Detail
        </BreadcrumbsItem>
  ...
  onTabClick = (tabValue) => {
    ...
    switch (tabValue) {
      case 1:
        ...
        break;
      case 2:
        this.props.trendingEventsForCinema(
          this.state.movieCategoryID,
          cinemaId
        );
        break;
      case 3:
        this.props.promotedEventsForCinema(
          this.state.movieCategoryID,
          cinemaId
        );
        break;
      case 4:
        this.props.upcomingEventsForCinema(
          this.state.movieCategoryID,
          cinemaId
        );
        break;
      ...
      <>
        {cinemaEvents.length > 0 ? (
          cinemaEvents.map((data, i) => {
            let categoryArr =
              data.categories &&
              (data.categories.includes([], 0)
                ? data.categories[0]
                : data.categories);
            let startDate = `${getDayFromISO(
              data.startDateTime && data.startDateTime
            )}, ${getDateFromISO(data.startDateTime && data.startDateTime)}`;
            let endDate = `${getDayFromISO(
              data.endDateTime && data.endDateTime
            )}, ${getDateFromISO(data.endDateTime && data.endDateTime)}`;
            const { currency } = data;
            return (
              <div
                className={"row"}
              ...
              >
                <div className={"col-md-9"}>
                  <CardWithSideDetail
                    ...
                    key={i}
                    title={data.title}
                    categories={categoryArr.map((category, j) => {
                      return (
                        <span key={j}>
                          {category.title}{" "}
                ...
                >
                  <button
                    className="simpleButton width85 backgroundColorRed"
          ...
          <div className={"row"} style={styles.innerDiv}>
            <div className={"col-md-6"}>
              <div className={"heading-text"}>
                ...
                >{`. ${totalEvents} Movies `}</span>{" "}
              </div>
            </div>
            <div
              className={"col-md-2 offset-4 cursor-pointer"}
              style={{ textAlign: "end" }}
            >
              <img
                src={
                  this.state.activeView === "list"
                    ? "/images/views/list-white.svg"
                    : "/images/views/list-gray.svg"
                }
              ...
              <img
                src={
                  this.state.activeView === "thumbnail"
                    ? "/images/views/calendar-white.svg"
                    : "/images/views/calendar - gray.svg"
                }
        ...
        <div className="content">
          ...
          >
            <div
              ...
              style={{
                float: "left",
                backgroundImage: `url('${cinemaData &&
                  cinemaData.defaultImage}')`,
                translateY: "30%",
              }}
            />
          ...
          {processing ? (
            ...
            this.renderCinemaDetail(totalEvents)
          )}
        </div>
      </div>
    );
