...
class CustomModal extends Component {
    ...
    render() {
        ...
        let tickets, passes, couponInfo, eventDetails, couponSign, transactionId, transactionAmount, type, paymentMethod,
            walletDeductedAmount, createdAt, lastBalance;
        ...
        if(this.props.orderItemDetails){
            ...
            couponSign = couponInfo && couponInfo.discountType === 'fixed' ? 'GHS' : '%';
        }
                        ...
                        <>
                            {
                                this.props.transactionDialogue ?
                                    <Modal isOpen={this.props.transactionDialogue} toggle2={this.toggle}
                                        ...
                                        <TransactionDialogue
                                            ...
                                            paymentType={type}
                                            couponApplied={couponInfo && (couponInfo.discountValue + couponSign)}
                                        />
                                    </Modal> :
                                    <div className='custom-modal custom-fade show custom-modal-open'
                                         onClick={(this.props.topUpAmount && this.props.successDialogue) ? () => {onCustomModalClick()} : null}
                                         role='dialog' tabIndex='-1'
                                         style={{display: 'block'}}>
                                        <div className={(this.props.topUpAmount && this.props.successDialogue) ?
                                            'custom-modal-dialog-top-margin' : 'custom-modal-dialog'}
                                             role='document'>
